    ...
    fetch(cvUrl, { method: 'HEAD' })
      .then(response => {
        if (response.ok) {
          ...
          showError('❌ CV file not found. Please add CV_M_Dzaka_Al_Fikri.pdf to the public folder.')
        }
      })
      .catch(() => {
        showError('❌ CV file not found. Please add CV_M_Dzaka_Al_Fikri.pdf to the public folder.')
          ...
          >
            ...
            >
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="bg-sky-600 hover:bg-sky-700 text-white px-8 py-3 rounded-lg font-semibold flex items-center gap-2 hover:shadow-lg hover:shadow-sky-500/30 transition-all justify-center"
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="border-2 border-sky-500 text-sky-400 px-8 py-3 rounded-lg font-semibold flex items-center gap-2 hover:bg-sky-500 hover:text-white transition-colors justify-center"
                onClick={downloadCV}
              >
                Download CV
                <Download size={20} />
